...
- **Correção pós-deploy:** logs de produção ainda falhavam PDF com `Cannot find module 'pdfjs-dist/package.json'` vindo de `/app/node_modules/pdf-to-img/dist/index.js`. `pdfjs-dist@5.4.624` agora também é dependência direta, entra em `serverExternalPackages`, `outputFileTracingIncludes` e tem fallback `COPY` explícito no `Dockerfile`.
...
- **Testes/validações locais:** `npm ls @napi-rs/canvas pdf-to-img pdfjs-dist` OK; `npm test` 34/34 verde; `npx tsc --noEmit` limpo; `npx eslint` nos arquivos tocados limpo; `docker build -t formulario-credfacil-docker-validate .` OK; dentro do container `require.resolve('pdfjs-dist/package.json')` OK, `import('pdf-to-img')` OK, `createCanvas(1,1)` OK; `/api/health` OK e container `healthy`.
